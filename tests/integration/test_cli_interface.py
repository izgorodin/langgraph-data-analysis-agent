...
class TestCLIInterface:
    ...
    def test_cli_basic_invocation(self, cli_runner, mock_graph_app, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_verbose_mode(self, cli_runner, mock_graph_app, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_custom_model(self, cli_runner, mock_graph_app, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_interactive_prompt(self, cli_runner, mock_graph_app, mock_bigquery_client, mock_gemini_client):
        """Test CLI interactive mode when no question provided."""
        with patch('cli.build_graph') as mock_build_graph:
            mock_build_graph.return_value = mock_graph_app
            
            # Simulate user input
            result = cli_runner.invoke(main, [], input='What is the revenue by category?\n')
            
            assert result.exit_code == 0
            assert "Enter your analysis question" in result.output
            assert "Insight" in result.output

    def test_cli_error_handling(self, cli_runner, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_json_output_format(self, cli_runner, mock_graph_app, mock_bigquery_client, mock_gemini_client):
        ...
        with patch('cli.build_graph') as mock_build_graph:
            ...
            for line in lines:
                if line.startswith('─') and any(node in line for node in ['plan', 'synthesize_sql', 'execute_sql']):
                    capturing = True
                    continue
    ...
    def test_cli_rich_output_formatting(self, cli_runner, mock_graph_app, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_long_output_truncation(self, cli_runner, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_no_report_handling(self, cli_runner, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_console_object(self, cli_runner, mock_graph_app, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_state_streaming(self, cli_runner, mock_bigquery_client, mock_gemini_client):
        ...
        with patch('cli.build_graph') as mock_build_graph:
            ...
            states = [
                ...
                {"execute_sql": {"sql": "SELECT * FROM test", "df_summary": {"rows": 5}}},
    ...
    def test_cli_graph_building(self, cli_runner, mock_bigquery_client, mock_gemini_client):
    ...
    def test_cli_state_model_dump_handling(self, cli_runner, mock_bigquery_client, mock_gemini_client):
