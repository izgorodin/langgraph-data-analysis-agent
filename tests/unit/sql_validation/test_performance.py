...
class TestValidationPerformance:
    ...
    def test_malicious_query_detection_performance(self):
        ...
        for query in malicious_queries:
            ...
            assert (
                validation_time < 0.1
            ), f"Malicious detection took {validation_time:.3f}s: {query}"
...
class TestMemoryUsage:
    ...
    def test_large_sql_memory_handling(self):
        ...
        large_where_clause = " OR ".join(
            [f"description LIKE '%pattern_{i}%'" for i in range(1000)]
        )
...
class TestConcurrencySupport:
    ...
    def test_parallel_validation_safety(self):
        ...
        for i, (index, error) in enumerate(results):
            ...
            assert (
                error == sequential_result.error
            ), f"Inconsistent result for query {index}"
...
class TestResourceLimits:
    ...
    def test_timeout_simulation(self):
        ...
        for i in range(10):  # Moderate nesting to avoid infinite parsing
            nested_query += (
                f"SELECT user_id FROM orders WHERE id > {i} AND user_id IN ("
            )
