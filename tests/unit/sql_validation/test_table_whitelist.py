    ...
    def test_schema_qualified_names_handled(self):
        ...
        for query in schema_queries:
            ...
            if result.error is None:
                ...
                assert any(keyword in result.error.lower() for keyword in ["table", "parse", "disallowed"])
...
class TestSystemTableAccess:
    ...
    def test_information_schema_blocked(self):
        ...
        for query in info_schema_queries:
            ...
            assert result.error is not None
            assert any(keyword in result.error.lower() for keyword in ["keyword", "forbidden", "information_schema"])
    ...
    def test_system_tables_blocked(self):
        ...
        for query in system_queries:
            ...
            assert result.error is not None
            assert any(keyword in result.error.lower() for keyword in ["keyword", "forbidden", "sys"])
