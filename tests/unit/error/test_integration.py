...
class TestErrorHandlingIntegration:
    ...
    async def test_bigquery_array_error_recovery(self):
        ...
        recovery = await engine.handle_error(
            error, context={"operation_id": "test_array"}
        )
    ...
    async def test_error_recovery_strategies(self):
        ...
        recovery = await engine.handle_error(
            timeout_error, context={"operation_id": "timeout_test"}
        )
        assert recovery.strategy == "immediate_retry"
        ...
        recovery = await engine.handle_error(
            rate_limit_error, context={"operation_id": "rate_test"}
        )
        ...
        recovery = await engine.handle_error(
            model_error, context={"operation_id": "model_test"}
        )
        assert recovery.strategy in [
            "model_fallback",
            "cached_fallback",
        ...
        recovery = await engine.handle_error(
            permission_error, context={"operation_id": "perm_test"}
        )
    ...
    async def test_error_context_preservation(self):
        ...
        recovery = await engine.handle_error(
            original_error,
            context={"operation_id": "context_test", "request_id": "req_789"},
        )
...
class TestErrorHandlingWithMockComponents:
    ...
    async def test_bigquery_integration_simulation(self):
        ...
        for error_msg, expected_strategy in errors_and_expected:
            ...
            recovery = await engine.handle_error(
                bq_error, context={"operation_id": f"bq_{hash(error_msg)}"}
            )
    ...
    async def test_llm_provider_error_simulation(self):
        ...
        provider_errors = [
            (
                "Model not found",
                "model_fallback",
            ),  # This should match the model pattern
        ...
        for error_msg, expected_strategy in provider_errors:
            ...
            recovery = await engine.handle_error(
                error, context={"operation_id": f"llm_{hash(error_msg)}"}
            )
