...
from src.error.recovery import RecoveryEngine, get_recovery_engine, set_recovery_engine
...
class TestRecoveryEngine:
    ...
    async def test_bigquery_array_error_handling(self, engine):
        ...
        error = BigQueryExecutionError(
            "Array cannot have a null element", query="SELECT ARRAY[1, NULL, 3] as arr"
        )
        ...
        assert recovery.modified_input is not None
        assert (
            "IGNORE NULLS" in recovery.modified_input
            or "IS NOT NULL" in recovery.modified_input
        )
    ...
    def test_bigquery_array_query_modification_no_array_error(self, engine):
        ...
        error = BigQueryExecutionError("Different error", query="SELECT * FROM table")
