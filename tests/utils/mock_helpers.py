...
class MockBigQueryHelper:
    ...
    @staticmethod
    def create_client_mock(schema_data: List[Dict] = None, query_results: pd.DataFrame = None):
    ...
    @staticmethod
    def create_error_client(error_type: str = "BadRequest", message: str = "Test error"):
...
class StateTestHelper:
    ...
    @staticmethod
    def create_error_state(question: str = "Error test", error: str = "Test error") -> AgentState:
        """Create an AgentState with an error condition."""
        return AgentState(question=question, error=error)
    
    @staticmethod
    def assert_state_progression(initial: AgentState, final: AgentState, expected_fields: List[str]):
        ...
        for field in expected_fields:
            assert getattr(final, field) is not None, f"Field {field} should be populated"
        
        # History should be maintained or expanded
        assert len(final.history) >= len(initial.history)
    
    @staticmethod
    def compare_states(state1: AgentState, state2: AgentState, ignore_fields: List[str] = None) -> bool:
...
class GraphTestHelper:
    ...
    @staticmethod
    def assert_node_execution_order(events: List[Dict[str, Any]], expected_order: List[str]):
        ...
        for node in actual_order:
            if expected_index < len(expected_order) and node == expected_order[expected_index]:
                expected_index += 1
        
        assert expected_index == len(expected_order), f"Expected order {expected_order}, got {actual_order}"
