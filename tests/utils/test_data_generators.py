...
class EcommerceDataGenerator:
    ...
    def generate_users(self, count: int = 100) -> pd.DataFrame:
        ...
        for i in range(count):
            user = {
                ...
                "traffic_source": random.choice(["Search", "Social", "Email", "Direct", "Referral"]),
    ...
    def generate_products(self, count: int = 50) -> pd.DataFrame:
        ...
        departments = ["Technology", "Fashion", "Home", "Outdoor", "Entertainment", "Personal Care", "Kids"]
        brands = ["BrandA", "BrandB", "BrandC", "TechCorp", "StyleCo", "HomeBase", "SportMax"]
    ...
    def generate_orders(self, users_df: pd.DataFrame, count: int = 200) -> pd.DataFrame:
        ...
        for i in range(count):
            ...
            status_weights = {"Complete": 0.65, "Processing": 0.15, "Shipped": 0.12, "Cancelled": 0.08}
            ...
            if status in ["Shipped", "Complete"]:
                ...
                if status == "Complete":
                    ...
                    if random.random() < 0.05:
                        returned_at = delivered_at + timedelta(days=random.randint(7, 30))
    ...
    def generate_order_items(self, orders_df: pd.DataFrame, products_df: pd.DataFrame, 
                           items_per_order_range: Tuple[int, int] = (1, 4)) -> pd.DataFrame:
        ...
        for _, order in orders_df.iterrows():
            ...
            for _, product in selected_products.iterrows():
                ...
                shipped_at = order["shipped_at"] if pd.notna(order["shipped_at"]) else None
                delivered_at = order["delivered_at"] if pd.notna(order["delivered_at"]) else None
                returned_at = order["returned_at"] if pd.notna(order["returned_at"]) else None
                ...
                if order["status"] == "Complete" and random.random() < 0.02:
                    ...
                    returned_at = fake.date_time_between(
                        start_date=delivered_at or created_at,
                        end_date=datetime.now()
                    )
...
class LLMResponseGenerator:
    ...
    def generate_analysis_plan(question: str, tables: List[str] = None) -> str:
        ...
        elif any(word in question_lower for word in ["customer", "user", "demographic"]):
            task_type = "customer_analysis"
            metrics = ["customer_count", "retention_rate", "lifetime_value"]
            relevant_tables = ["users", "orders"]
        elif any(word in question_lower for word in ["product", "inventory", "performance"]):
    ...
    def generate_business_report(question: str, df_summary: Dict[str, Any]) -> str:
        ...
        report_parts.append(f"Analysis completed on {rows} records across {len(columns)} metrics.")
        ...
        if head_data:
            ...
            if "revenue" in sample_row:
                ...
                report_parts.append(f"Total revenue from top categories: ${total_revenue:,.2f}")
        ...
        # Key Findings
        report_parts.append("\n### Key Findings")
        report_parts.append("• Strong performance indicators across all measured dimensions")
        report_parts.append("• Clear patterns emerge in the data suggesting actionable opportunities")
        ...
        report_parts.append("2. Investigate underperforming areas for improvement opportunities")
...
class ConfigurationGenerator:
    ...
    def generate_bigquery_schema() -> List[Dict[str, str]]:
        ...
        # Orders table
        orders_columns = [
            ("order_id", "INTEGER"), ("user_id", "INTEGER"), ("status", "STRING"),
            ("created_at", "TIMESTAMP"), ("shipped_at", "TIMESTAMP"), 
            ("delivered_at", "TIMESTAMP"), ("returned_at", "TIMESTAMP"), ("num_of_item", "INTEGER")
        ]
        ...
        # Products table
        products_columns = [
            ("id", "INTEGER"), ("cost", "FLOAT"), ("category", "STRING"),
            ("name", "STRING"), ("brand", "STRING"), ("retail_price", "FLOAT"),
            ("department", "STRING"), ("sku", "STRING"), ("distribution_center_id", "INTEGER")
        ]
        ...
        users_columns = [
            ...
            ("email", "STRING"), ("age", "INTEGER"), ("gender", "STRING"),
