...
from ..llm.models import LLMRequest, LLMContext
...
class LLMNodeIntegration:
    ...
    async def generate_report(self, question: str, plan_json: Dict, df_summary: Dict) -> str:
        """Generate analysis report using LLM with proper context."""
        plan_text = json.dumps(plan_json, ensure_ascii=False)
        summary_text = json.dumps(df_summary, ensure_ascii=False)[:20000]  # Truncate for token limits
    ...
    def generate_report_sync(self, question: str, plan_json: Dict, df_summary: Dict) -> str:
