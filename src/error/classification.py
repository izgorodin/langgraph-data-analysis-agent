...
class ErrorClassifier:
    ...
    def __init__(self):
        ...
        self._patterns = [
            ...
            (r"permission.*denied|access.*denied|unauthorized|invalid.*api.*key|authentication.*failed|forbidden", RecoveryStrategy.NO_RECOVERY, ErrorSeverity.CRITICAL),
            
            # Network/timeout errors - immediate retry
            (r"timeout|connection.*reset|network.*error", RecoveryStrategy.IMMEDIATE_RETRY, ErrorSeverity.MEDIUM),
            (r"rate.*limit|quota.*exceeded|too.*many.*requests", RecoveryStrategy.EXPONENTIAL_BACKOFF, ErrorSeverity.MEDIUM),
            
            # BigQuery specific errors
            (r"Array cannot have a null element", RecoveryStrategy.IMMEDIATE_RETRY, ErrorSeverity.MEDIUM),
            (r"dataset.*not.*found", RecoveryStrategy.NO_RECOVERY, ErrorSeverity.HIGH),
            
            # LLM provider errors  
            (r"model.*not.*found|model.*unavailable", RecoveryStrategy.GRACEFUL_DEGRADATION, ErrorSeverity.HIGH),
            
            # SQL and schema errors - user guided
            (r"syntax.*error|invalid.*sql", RecoveryStrategy.USER_GUIDED, ErrorSeverity.HIGH),
            (r"table.*not.*found|column.*not.*found", RecoveryStrategy.USER_GUIDED, ErrorSeverity.MEDIUM),
            
            # System errors
            (r"out.*of.*memory|disk.*full", RecoveryStrategy.GRACEFUL_DEGRADATION, ErrorSeverity.HIGH),
            (r"internal.*server.*error", RecoveryStrategy.EXPONENTIAL_BACKOFF, ErrorSeverity.MEDIUM),
        ]
    
    def classify(self, error: Union[Exception, str]) -> tuple[RecoveryStrategy, ErrorSeverity]:
        """
    ...
    def _classify_lgda_error(self, error: LGDAError) -> tuple[RecoveryStrategy, ErrorSeverity]:
        """Classify LGDA-specific errors."""
        from .core import SqlGenerationError, BigQueryExecutionError, TimeoutError
        ...
        if isinstance(error, TimeoutError):
            ...
            # For other BigQuery errors, use pattern-based classification
            return self._classify_by_patterns(error.message)
        
        return RecoveryStrategy.USER_GUIDED, ErrorSeverity.MEDIUM
    
    def _classify_by_patterns(self, error_message: str) -> tuple[RecoveryStrategy, ErrorSeverity]:
    ...
    def get_user_message(self, error: Union[Exception, str]) -> str:
        ...
        if "table not found" in error_message.lower() or "column not found" in error_message.lower():
            return "Unable to complete request. Please check your table or column names."
