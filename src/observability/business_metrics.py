...
class BusinessMetrics:
    ...
    def _record_metric(
        ...
        try:
            ...
            logger.info(
                f"Business metric: {metric_name}={value}, dimensions={dimensions}"
            )

        except Exception as e:
            logger.error(f"Failed to record business metric {metric_name}: {e}")

    def track_query_success_rate(
        self,
        success: bool,
        question: str = "",
    ...
    def track_resource_efficiency(
        ...
        efficiency_ratio = insight_value / max(
            query_cost, 0.001
    ...
    def track_error_patterns(
        ...
        self._record_metric(
            ...
            metadata={"error_frequency": error_frequency},
        )
    ...
    def track_feature_usage(
        ...
        self._record_metric(
            ...
            dimensions={
                ...
                "success": str(success).lower(),
                "satisfaction_level": (
                    self._categorize_satisfaction(user_satisfaction)
                    if user_satisfaction
                    else "unknown"
                ),
            },
            metadata={"satisfaction_score": user_satisfaction},
        )

    def track_business_kpi(
        self,
        kpi_name: str,
        value: float,
        ...
        if target_value is not None:
            ...
            metadata["target_achievement"] = (
                value / target_value if target_value > 0 else 0
            )

        self._record_metric(
            f"business_kpi_{kpi_name}", value, dimensions=dimensions, metadata=metadata
        )
    ...
    def get_session_summary(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "questions_per_minute": session["question_count"]
            / max((current_time - session["start_time"]) / 60, 0.1),
        }
