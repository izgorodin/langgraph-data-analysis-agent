...
def validate_task_file(file_path: Path) -> List[str]:
    ...
    if "performance" not in content.lower() and "производительност" not in content.lower():
...
def main():
    ...
    found_ids = {re.search(r'LGDA-\d{3}', f.name).group() for f in task_files if re.search(r'LGDA-\d{3}', f.name)}
